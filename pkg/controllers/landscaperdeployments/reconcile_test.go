...
var _ = Describe("SortServiceTargetConfigs", func() {
	It("should sort descending by priority", func() {
		configs := &provisioningv1alpha2.ServiceTargetConfigList{
			Items: []provisioningv1alpha2.ServiceTargetConfig{
				{
					Spec: provisioningv1alpha2.ServiceTargetConfigSpec{
						Priority: 20,
					},
				},
				{
			...
			Items: []provisioningv1alpha2.ServiceTargetConfig{
				{
					...
					Status: provisioningv1alpha2.ServiceTargetConfigStatus{
						InstanceRefs: []provisioningv1alpha2.ObjectReference{
							{
								Name:      "foo",
								Namespace: "bar",
							},
...
var _ = Describe("Reconcile", func() {
	...
	It("should set finalizer and update observed generation", func() {
		...
		Expect(kutil.HasFinalizer(deployment, constants.LandscaperServiceFinalizer)).To(BeTrue())
	...
	It("should handle reconcile errors", func() {
		...
		ctrl.ReconcileFunc = func(ctx context.Context, deployment *provisioningv1alpha2.LandscaperDeployment) error {
	...
	It("should respect the ignore operation annotation", func() {
		...
		err = testenv.Client.Get(ctx, types.NamespacedName{Name: deployment.Status.InstanceRef.Name, Namespace: deployment.Status.InstanceRef.Namespace}, instance)
		Expect(err).ToNot(HaveOccurred())
		Expect(utils.HasOperationAnnotation(instance, constants.LandscaperServiceOperationIgnore)).To(BeTrue())
		...
		err = testenv.Client.Get(ctx, types.NamespacedName{Name: deployment.Status.InstanceRef.Name, Namespace: deployment.Status.InstanceRef.Namespace}, instance)
		Expect(err).ToNot(HaveOccurred())
		Expect(utils.HasOperationAnnotation(instance, constants.LandscaperServiceOperationIgnore)).To(BeFalse())
	})
})
